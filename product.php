<?php
session_start();
$network = isset($_SESSION['network_type']) ? $_SESSION['network_type'] : '4g';
$imagePath = "images/products/";
$productImage = "product123.jpg"; // Change this to the dynamic product image variable

// Define different quality images
$lowQuality = $imagePath . "low/" . $productImage;
$mediumQuality = $imagePath . "medium/" . $productImage;
$highQuality = $imagePath . "high/" . $productImage;

if ($network == '2g' || $network == 'slow-2g') {
    $optimizedImage = $lowQuality;
} elseif ($network == '3g') {
    $optimizedImage = $mediumQuality;
} else {
    $optimizedImage = $highQuality;
}
...
<div class="page">
    ...
    <span>
    <?php if($p_old_price!=''): ?>
        <del id="old_price">Rs. <?php echo number_format($p_old_price, 2); ?></del>
    <?php endif; ?> 
    Rs. <strong id="current_price"><?php echo number_format($p_current_price, 2); ?></strong>
    </span>
</div>

<!-- Hidden fields for JS to use base unit prices -->
<input type="hidden" id="unit_old_price" value="<?php echo $p_old_price; ?>">
<input type="hidden" id="unit_current_price" value="<?php echo $p_current_price; ?>">
...
document.addEventListener('DOMContentLoaded', function () {
    const qtyInput = document.querySelector('input[name="p_qty"]');
    const oldPrice = document.getElementById('old_price');
    const currentPrice = document.getElementById('current_price');
    const unitOld = parseFloat(document.getElementById('unit_old_price')?.value || 0);
    const unitCurrent = parseFloat(document.getElementById('unit_current_price')?.value || 0);

    function updatePrices() {
        const qty = parseInt(qtyInput.value) || 1;
        if (oldPrice) oldPrice.textContent = 'Rs. ' + (unitOld * qty).toFixed(2);
        if (currentPrice) currentPrice.textContent = (unitCurrent * qty).toFixed(2);
    }

    qtyInput.addEventListener('input', updatePrices);
    updatePrices(); // update on load
});
</script>
