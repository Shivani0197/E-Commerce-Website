...
<?php
// Log session duration when user is logged in and session start time is available
if (isset($_SESSION['customer']) && isset($_SESSION['session_start_time'])) {
    $session_duration_seconds = time() - $_SESSION['session_start_time'];
    $duration_formatted = gmdate("H:i:s", $session_duration_seconds);
    $email = $_SESSION['customer']['cust_email'];

    // Check if already logged for this session
    if (!isset($_SESSION['duration_logged'])) {
        $statement = $pdo->prepare("INSERT INTO tbl_user_activity (user_email, action_type, action_data, action_time, session_duration) VALUES (?, ?, ?, NOW(), ?)");
        $statement->execute([
            $email,
            'Session Duration',
            'User spent '.$duration_formatted.' on website',
            $duration_formatted
        ]);
        $_SESSION['duration_logged'] = true; // mark this session as logged
    }
}
?>
